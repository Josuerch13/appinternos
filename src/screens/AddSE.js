...
export default function AddSE() {
    ...
    return(
        <RN.View style={styles.container}>
            <RN.Text style={styles.title}>Solicitar salida para fin de semana</RN.Text>
                <RN.TextInput 
                    onChangeText={(text) => setNewItem({...newItem, name: text})}
                    style={styles.inputContainer} 
                    placeholder='Nombre' 
                />
                <RN.Text style={styles.TextTitle}>Cuando Saldras?</RN.Text>
                <RN.TouchableOpacity  style={styles.button}
                title="Seleccionar Fecha De salida" onPress= {showDatePicker} > 
                <RN.Text style={styles.buttonText}>Seleccionar Fecha de Salida</RN.Text>
                </RN.TouchableOpacity> 
      <DateTimePickerModal
        isVisible={isDatePickerVisible}
        mode="date"
        onConfirm={(handleConfirm) =>setNewItem({...newItem, fechaSalida: handleConfirm})} 
        onCancel={hideDatePicker}
      />
      <RN.Text style={styles.TextTitle}>Cuando Regresaras?</RN.Text>
      <RN.TouchableOpacity  style={styles.button}
                title="Seleccionar Fecha de Retorno" onPress={showDatePicker} > 
                <RN.Text style={styles.buttonText}>Seleccionar Fecha de Retorno</RN.Text>
                </RN.TouchableOpacity> 
      <DateTimePickerModal
        isVisible={isDatePickerVisible}
        mode="date"
        onConfirm={(handleConfirm2) =>setNewItem({...newItem, fechaRetorno: handleConfirm2})} 
        onCancel={hideDatePicker}
      />
